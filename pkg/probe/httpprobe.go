...
func prepareHTTPProbe(ctx context.Context, probe v1alpha1.ProbeAttributes, clients clients.ClientSets, chaosDetails *types.ChaosDetails, resultDetails *types.ResultDetails, phase string) error {

	switch strings.ToLower(phase) {
	case "prechaos":
		if err := preChaosHTTPProbe(ctx, probe, resultDetails, clients, chaosDetails); err != nil {
			return err
		}
	case "postchaos":
		if err := postChaosHTTPProbe(ctx, probe, resultDetails, chaosDetails.Delay, chaosDetails.Timeout); err != nil {
			return err
		}
	case "duringchaos":
		onChaosHTTPProbe(ctx, probe, resultDetails, clients, chaosDetails)
	default:
...
func triggerHTTPProbe(ctx context.Context, probe v1alpha1.ProbeAttributes, resultDetails *types.ResultDetails) error {
	...
	switch method {
	case "Get":
		log.WithContext(ctx).InfoWithValues("[Probe]: HTTP get method informations", logrus.Fields{
			"Name":            probe.Name,
		...
		return httpGet(ctx, probe, client, resultDetails)
	case "Post":
		log.WithContext(ctx).InfoWithValues("[Probe]: HTTP Post method informations", logrus.Fields{
			"Name":            probe.Name,
			"URL":             probe.HTTPProbeInputs.URL,
		...
		return httpPost(ctx, probe, client, resultDetails)
	}
	return nil
}
...
func httpGet(ctx context.Context, probe v1alpha1.ProbeAttributes, client *http.Client, resultDetails *types.ResultDetails) error {
	probeTimeout := getProbeTimeouts(probe.Name, resultDetails.ProbeDetails)
	...
	if err := retry.Times(uint(getAttempts(probe.RunProperties.Attempt, probe.RunProperties.Retry))).
		...
		Try(func(attempt uint) error {
			...
			if err = cmp.RunCount(rc).
				...
				log.WithContext(ctx).Errorf("The %v http probe get method has Failed, err: %v", probe.Name, err)
				return err
			}
...
func httpPost(ctx context.Context, probe v1alpha1.ProbeAttributes, client *http.Client, resultDetails *types.ResultDetails) error {
	probeTimeout := getProbeTimeouts(probe.Name, resultDetails.ProbeDetails)
	...
	if err := retry.Times(uint(getAttempts(probe.RunProperties.Attempt, probe.RunProperties.Retry))).
		...
		Try(func(attempt uint) error {
			...
			if err = cmp.RunCount(rc).
				...
				ProbeVerbosity(probe.RunProperties.Verbosity).
				CompareInt(cerrors.FailureTypeHttpProbe); err != nil {
				log.WithContext(ctx).Errorf("The %v http probe post method has Failed, err: %v", probe.Name, err)
				return err
			}
			description = fmt.Sprintf("The URL %s did respond with correct status code. Actual code: '%s'. Expected code: '%s'", probe.HTTPProbeInputs.URL, code, probe.HTTPProbeInputs.Method.Get.ResponseCode)
			return nil
...
func triggerContinuousHTTPProbe(ctx context.Context, probe v1alpha1.ProbeAttributes, clients clients.ClientSets, chaosresult *types.ResultDetails, chaosDetails *types.ChaosDetails) {
...
loop:
	for {
		...
		case <-chaosDetails.ProbeContext.Ctx.Done():
			...
			break loop
		default:
			err = triggerHTTPProbe(ctx, probe, chaosresult)
			...
			if err != nil {
				...
				for index := range chaosresult.ProbeDetails {
					if chaosresult.ProbeDetails[index].Name == probe.Name {
						...
						log.WithContext(ctx).Errorf("The %v http probe has been Failed, err: %v", probe.Name, err)
...
func preChaosHTTPProbe(ctx context.Context, probe v1alpha1.ProbeAttributes, resultDetails *types.ResultDetails, clients clients.ClientSets, chaosDetails *types.ChaosDetails) error {
	probeTimeout := getProbeTimeouts(probe.Name, resultDetails.ProbeDetails)
	...
	case "SOT", "Edge":
		...
		log.WithContext(ctx).InfoWithValues("[Probe]: The http probe information is as follows", logrus.Fields{
			"Name":           probe.Name,
			"URL":            probe.HTTPProbeInputs.URL,
		...
		if err = triggerHTTPProbe(ctx, probe, resultDetails); err != nil && cerrors.GetErrorType(err) != cerrors.FailureTypeHttpProbe {
			return err
		}
	...
	case "Continuous":
		...
		go triggerContinuousHTTPProbe(ctx, probe, clients, resultDetails, chaosDetails)
...
func postChaosHTTPProbe(ctx context.Context, probe v1alpha1.ProbeAttributes, resultDetails *types.ResultDetails, delay int, timeout int) error {
	probeTimeout := getProbeTimeouts(probe.Name, resultDetails.ProbeDetails)
	...
	case "Continuous", "OnChaos":
		...
		if err = checkForErrorInContinuousProbe(ctx, resultDetails, probe.Name, delay, timeout); err != nil && cerrors.GetErrorType(err) != cerrors.FailureTypeHttpProbe && cerrors.GetErrorType(err) != cerrors.FailureTypeProbeTimeout {
			return err
		}
		// failing the probe, if the success condition doesn't met after the retry & timeout combinations
		if err = markedVerdictInEnd(ctx, err, resultDetails, probe, "PostChaos"); err != nil {
...
func triggerOnChaosHTTPProbe(ctx context.Context, probe v1alpha1.ProbeAttributes, clients clients.ClientSets, chaosresult *types.ResultDetails, chaosDetails *types.ChaosDetails) {
	probeTimeout := getProbeTimeouts(probe.Name, chaosresult.ProbeDetails)
...
func onChaosHTTPProbe(ctx context.Context, probe v1alpha1.ProbeAttributes, resultDetails *types.ResultDetails, clients clients.ClientSets, chaosDetails *types.ChaosDetails) {
	...
	case "OnChaos":
		...
		go triggerOnChaosHTTPProbe(ctx, probe, clients, resultDetails, chaosDetails)
	}
